...
class SaveAsForm(QtGui.QWidget):
    ...
    def __init__(self, handler, preview_updater, is_publish, name, parent = None):
        """
        Construction
        """
    ...
    def _enable_controls(self, val=True):
        """
        Enables or disables all the input controls.
        """
    ...
    def _change_work_area(self):
        """
        Changes the work area the file will be saved into.
        """
        # disable controls while the change work area window
        # is open.
        self._enable_controls(False)

        # call the change work area method on
        # SaveAs handler.
        self._handler.change_work_area()

        # not sure if this is the best way but we want to wait
        # a moment while the change work area window closes.
        QtCore.QTimer.singleShot(200, self._update_preview_info)

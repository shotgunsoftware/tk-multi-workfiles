...
class WorkFiles(object):
    ...
    def _execute_scene_operation_hook(self, operation, file_path=None):
        return self._app.execute_hook("hook_scene_operation", operation=operation, file_path=file_path, context = self._context)
    ...
    def _reset_current_scene(self):
        ...
        while self._execute_scene_operation_hook("scene_modified"):
            ...
            if res == QtGui.QMessageBox.Cancel:
                ...
                scene_name = self._execute_scene_operation_hook("current_path")
                try:
                    if scene_name:
                        snapshot = self._app.engine.commands.get("Snapshot...")
                        res = snapshot.get("callback")()
                        if not res:
                            return False
                    else:
                        tank_save = self._app.engine.commands.get("Tank Save As...")
                        res = tank_save.get("callback")()
                        if not res:
                            return False
                except:
                    if scene_name:
                        res = self._execute_scene_operation_hook("save", file_path=scene_name)
                    else:
                        res = self._execute_scene_operation_hook("save_as")
        ...
        # Now do the scene reset...
        res = self._execute_scene_operation_hook("reset")
        if res == None or not isinstance(res, bool):
            raise TankError("Unexpected type returned from 'hook_scene_operation' - expected 'bool' but returned '%s'" % type(res).__name__)
        return res
    ...
    def _open_file(self, path):
        ...
        """
        # do open:
        self._execute_scene_operation_hook("open", file_path=path)
    ...
    def _save_file(self):
        ...
        """
        self._execute_scene_operation_hook("save")
